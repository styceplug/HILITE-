...
class _OthersProfileState extends State<OthersProfileScreen> {
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        Text(
                          user.role == 'club'
                              ? (club?.clubName ?? user.name)
                              : user.role == 'agent'
                              ? user.agentDetails?.agencyName ?? user.name
                                    ...
                                    child: ElevatedButton.icon(
                                      onPressed:
                                          isLoadingProfile
                                              ...
                                              : () {
                                                final link =
                                                    "https://hiliteapp.net/profile/${user.id}";
                                                Share.share(
                                                  'Check out my profile on Hilite! $link',
                                                );
                                              },
                                      icon: const Icon(
                                        Icons.share,
  ...
  Widget _buildSelectedTabContent(
    ...
    if (currentTab == 'Highlights') {
      final bool isLoadingPosts =
          controller.isExternalPostsLoading && controller.externalPosts.isEmpty;
      if (isLoadingPosts || isLoading) {
        return _buildCombinedContentGrid(
          controller,
          user,
          dummyPosts: _getMockPosts(),
        );
      }
          ...
          if (user.role == 'player') ...[
            ...
            Row(
              ...
              children: [
                ...
                _buildPlayerStat(
                  'Foot',
                  player?.preferredFoot.capitalizeFirst ?? '-',
                ),
              ],
            ),
          ],
  ...
  // --- GRID RENDERER ---
  Widget _buildCombinedContentGrid(
    UserController controller,
    UserModel user, {
    List<PersonalPostModel>? dummyPosts,
  }) {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return GestureDetector(
          onTap: dummyPosts != null ? null : () async {
            final converted = posts.map((p) => personalToPostModel(p, authorProfile: controller.othersProfile.value)).toList();
            final tappedIndex = posts.indexWhere((p) => p.id == post.id);

            await Get.to(() => ProfileReelsPlayer(
              videos: converted,
              initialIndex: tappedIndex == -1 ? 0 : tappedIndex,
              authorProfile: controller.othersProfile.value,
            ));

            final targetId = controller.othersProfile.value?.id;
            if (targetId != null) {
              controller.getAllExternalUserPosts(targetId);
            }
          },
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                ...
                const Expanded(
                  child: Text(
                    "Open to Trials & Recruiting Players",
                    style: TextStyle(
                      color: Colors.white,
                      fontWeight: FontWeight.w600,
                    ),
          ...
          ...List.generate(
            4,
            (index) => _buildSquadTile("Adebiyi Adebayo", "CAM • 22 • Nigeria"),
          ),
        ],
      ),
    );
  ...
  UserModel _getMockUser() {
    return UserModel(
      ...
      posts: 0,
      bio:
          'I am a dedicated football scout with a strong eye for identifying raw talent, tactical intelligence, and long-term player potential...',
